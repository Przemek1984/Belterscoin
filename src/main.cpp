...
#if defined(NDEBUG)
# error "Belterscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xad593b0f64f86ad85415f14131740d42af869c3d8ffc01a3137fadee1b060497");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Belterscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Belterscoin Signed Message:\n";
...
{
    // Belterscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 150000); // Belterscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Belterscoin: 1 day
static const int64 nTargetSpacing = 10 * 60; // Belterscoin: 10 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Belterscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Belterscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x28d58ccd300f1ff188f32f3505a648d330cf27bde274a7acc2c4351c726cd466");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "My cats went to hospital";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040c47282aa94fd5538273420a5da61fca940b730d6c9c6c9d1578c374f96ffbdd36e200b89f57e95d033f38aa56489387e92b636cc5719cc780b912bee0ac4dfc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1529318373;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086718068;

        if (fTestNet)
        {
            block.nTime    = 1529318331;
            block.nNonce   = 385615349;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x19806da1ca73befffed0f399b97d60c59f86e86abf9db31daa96c8ceba9b7065"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc8, 0xb1, 0xdd }; // Belterscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BelterscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BelterscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BelterscoinMiner(CWallet *pwallet)
{
    printf("BelterscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("belterscoin-miner");
    ...
    try { loop {
        ...
        printf("Running BelterscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BelterscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BelterscoinMiner, pwallet));
}
